import React from 'react'
import './MenFootwear.css'
import KeyboardArrowDownIcon from '@mui/icons-material/KeyboardArrowDown';

function MenFootwear() {
    return (
       <div className='navs'>

         <div className='nave'>

           <div className='one-opt'>
             <span className='one'>Electronics <KeyboardArrowDownIcon fontSize="small" className="down-iscon"/></span>
             <div className='down-side'>
                <div className='down-hill'>
                    <h6 className='sire'>Mobile</h6>

                    <li><a href="/">Mi</a></li>
                    <li><a href="/">Realme</a></li>
                    <li><a href="/">Samsung</a></li>
                    <li><a href="/">OPPO</a></li>
                    <li><a href="/">Apple</a></li>
                    <li><a href="/">Vivo</a></li>
                    <li><a href="/">Asux</a></li>
                    <li><a href="/">Poco X2</a></li>
            
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Mobile Accessories</h6>

                    <li><a href="/">Mobile Cases</a></li>
                    <li><a href="/">Power Banks</a></li>
                    <li><a href="/">Memmory Cards</a></li>
                    <li><a href="/">Mobiles Cables</a></li>
                    <li><a href="/">Mobiles Charges</a></li>
                    <li><a href="/">Mobile Holders</a></li>
                    <li><a href="/">Smart Watches</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Laptops</h6>

                    <li><a href="/">Gaming Laptops</a></li>
                    <li><a href="/">Pendrives</a></li>
                    <li><a href="/">Lptop Bags</a></li>
                    <li><a href="/">Mouse</a></li>
                    <li><a href="/">Decals</a></li>
                    <li><a href="/">Laptop Skins</a></li>
                    <li><a href="/">Monitors</a></li>
                    <li><a href="/">Apple iPads</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Featured</h6>

                    <li><a href="/">Flipkart SmartBuy</a></li>
                    <li><a href="/">Apple Products</a></li>
                    <li><a href="/">Microsoft Stores</a></li>
                    <li><a href="/">JBL Speakers</a></li>
                    <li><a href="/">Philips</a></li>
                    <li><a href="/">Dr Morepen</a></li>
                    <li><a href="/">Mobile No Cost EMI</a></li>
                    <li><a href="/">Google Assistant Stores</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Television</h6>

                    <li><a href="/">Speaker</a></li>
                    <li><a href="/">Home Theatres</a></li>
                    <li><a href="/">Soundbars</a></li>
                    <li><a href="/">Bluetooth Speakers</a></li>
                    <li><a href="/">Google Nest</a></li>
                    <li><a href="/">Camera</a></li>
                    <li><a href="/">Lens</a></li>
                    <li><a href="/">Tripods</a></li>
                </div>
             </div>
           </div>

           <div className='one-opt'>
             <span className='two'>TVs & Appliances <KeyboardArrowDownIcon fontSize="small" className="down-iscon"/></span>
             <div className='down-side'>
                <div className='down-hill'>
                    <h6 className='sire'>Top Brands</h6>

                    <li><a href="/">Mi</a></li>
                    <li><a href="/">Yu</a></li>
                    <li><a href="/">Thomson</a></li>
                    <li><a href="/">Samsung</a></li>
                    <li><a href="/">Nokia</a></li>
                    <li><a href="/">LG</a></li>
                    <li><a href="/">Realme</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Air Conditioner</h6>

                    <li><a href="/">Inverter AC</a></li>
                    <li><a href="/">Split ACs</a></li>
                    <li><a href="/">Window ACs</a></li>
                    <li><a href="/">LG</a></li>
                    <li><a href="/">Carrier</a></li>
                    <li><a href="/">Single Door</a></li>
                    <li><a href="/">Double Door</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Kitchen Appliances</h6>

                    <li><a href="/">Microwave Ovens</a></li>
                    <li><a href="/">Electric Kettle</a></li>
                    <li><a href="/">Chimneys</a></li>
                    <li><a href="/">Hand Blanders</a></li>
                    <li><a href="/">Sandwhich Makers</a></li>
                    <li><a href="/">Electric Cookers</a></li>
                    <li><a href="/">Wet Grinders</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Small Home Appliances</h6>

                    <li><a href="/">Irons</a></li>
                    <li><a href="/">Fans</a></li>
                    <li><a href="/">Air Coolers</a></li>
                    <li><a href="/">Water Purifies</a></li>
                    <li><a href="/">Inverters</a></li>
                    <li><a href="/">Vaccum Cleaners</a></li>
                    <li><a href="/">Sewing Machines</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Buying Guides</h6>

                    <li><a href="/">Televisions</a></li>
                    <li><a href="/">Air Conditioners</a></li>
                    <li><a href="/">Refrigerators</a></li>
                    <li><a href="/">Waching Machines</a></li>
                    <li><a href="/">Water Purifies</a></li>
                    <li><a href="/">Air Purifies</a></li>
                    <li><a href="/">Water Geysers</a></li>
                </div>
             </div>
           </div>

           <div className='one-opt'>
             <span className='three'>Men<KeyboardArrowDownIcon fontSize="small" className="down-iscon"/></span>
             <div className='down-side'>
                <div className='down-hill'>
                    <h6 className='sire'>Footwear</h6>

                    <li><a href="/">Sports shoes</a></li>
                    <li><a href="/">Casual Shoes</a></li>
                    <li><a href="/">Formal Shoes</a></li>
                    <li><a href="/">Flip-Flop</a></li>
                    <li><a href="/">Loffers</a></li>
                    <li><a href="/">Boots</a></li>
                    <li><a href="/">Sneakers</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Clothing</h6>

                    <li><a href="/">T-shirts</a></li>
                    <li><a href="/">Formal shirts</a></li>
                    <li><a href="/">Casual Shirts</a></li>
                    <li><a href="/">Jeans</a></li>
                    <li><a href="/">Cargos</a></li>
                    <li><a href="/">Casual Trouser</a></li>
                    <li><a href="/">Shorts</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Winter Wear</h6>

                    <li><a href="/">SweatShirts</a></li>
                    <li><a href="/">Jackets</a></li>
                    <li><a href="/">Sweater</a></li>
                    <li><a href="/">Tracksuits</a></li>
                    <li><a href="/">Kurta</a></li>
                    <li><a href="/">Ethnic Sets</a></li>
                    <li><a href="/">Lungi</a></li>
                </div>

                <div className='down-hill'>
                <h6 className='sire'>Featured</h6>

                  <li><a href="/">Watches Store</a></li>
                 <li><a href="/">Footwear Club</a></li>
                  <li><a href="/">Bags & Wallets</a></li>
                   <li><a href="/">Adidas</a></li>
                   <li><a href="/">Reebok</a></li>
                 <li><a href="/">Skechers</a></li>
                    <li><a href="/">Nike</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Watches</h6>

                    <li><a href="/">Fastrack</a></li>
                    <li><a href="/">Casio</a></li>
                    <li><a href="/">Titan</a></li>
                    <li><a href="/">Fossil</a></li>
                    <li><a href="/">Sonata</a></li>
                    <li><a href="/">Wallets</a></li>
                    <li><a href="/">Belts</a></li>
                </div>
             </div>
           </div>

           <div className='one-opt'>
             <span className='four'>Women<KeyboardArrowDownIcon fontSize="small" className="down-iscon"/></span>
             <div className='down-side'>
                <div className='down-hill'>
                    <h6 className='sire'>Clothing</h6>

                    <li><a href="/">Topwear</a></li>
                    <li><a href="/">Dresses</a></li>
                    <li><a href="/">Shorts</a></li>
                    <li><a href="/">Jeans</a></li>
                    <li><a href="/">Skirts</a></li>
                    <li><a href="/">Jegging & Tights</a></li>
                    <li><a href="/">Trousers & Capris</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Ethnic Wear</h6>

                    <li><a href="/">Sarees</a></li>
                    <li><a href="/">Kurta & kurtis</a></li>
                    <li><a href="/">Dress Materials</a></li>
                    <li><a href="/">Lehenga Choli</a></li>
                    <li><a href="/">Blouse</a></li>
                    <li><a href="/">Gowns</a></li>
                    <li><a href="/">Dupattas</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Footwear</h6>

                    <li><a href="/">Flat</a></li>
                    <li><a href="/">Heels</a></li>
                    <li><a href="/">Wedges</a></li>
                    <li><a href="/">Boots</a></li>
                    <li><a href="/">Sport Shoes</a></li>
                    <li><a href="/">Casual Shoes</a></li>
                    <li><a href="/">Slippers</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Beauty & Grooming</h6>

                    <li><a href="/">Make Up</a></li>
                    <li><a href="/">Skin Care</a></li>
                    <li><a href="/">Hair Care</a></li>
                    <li><a href="/">Bath</a></li>
                    <li><a href="/">Spa</a></li>
                    <li><a href="/">Deodrants</a></li>
                    <li><a href="/">Perfumes</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Featured</h6>

                    <li><a href="/">Forever 21</a></li>
                    <li><a href="/">Accessorize</a></li>
                    <li><a href="/">Chemistry</a></li>
                    <li><a href="/">Lakme</a></li>
                    <li><a href="/">Nivea</a></li>
                    <li><a href="/">Frastrack</a></li>
                    <li><a href="/">Crocs</a></li>
                </div>
             </div>
           </div>

           <div className='one-opt'>
             <span className='five'>Baby & Kids<KeyboardArrowDownIcon fontSize="small" className="down-iscon"/></span>
             <div className='down-side'>
                <div className='down-hill'>
                    <h6 className='sire'>Kids Clothing</h6>

                    <li><a href="/">T-shirts</a></li>
                    <li><a href="/">Shorts</a></li>
                    <li><a href="/">Ethnic Wear</a></li>
                    <li><a href="/">Shirts</a></li>
                    <li><a href="/">Innerwear</a></li>
                    <li><a href="/">Jackets</a></li>
                    <li><a href="/">Pants</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Kid's Footwear</h6>

                    <li><a href="/">Sandals</a></li>
                    <li><a href="/">Sport Shoes</a></li>
                    <li><a href="/">Kids Watches</a></li>
                    <li><a href="/">Boys Sweatshirts</a></li>
                    <li><a href="/">Boys Jacket</a></li>
                    <li><a href="/">Kids Sunglasses</a></li>
                    <li><a href="/">Flats & Bellies</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Toys</h6>

                    <li><a href="/">Remote Control Toys</a></li>
                    <li><a href="/">Educational Toys</a></li>
                    <li><a href="/">Soft Toys</a></li>
                    <li><a href="/">Outdoor Toys</a></li>
                    <li><a href="/">Action Figures</a></li>
                    <li><a href="/">Board Games</a></li>
                    <li><a href="/">Musical Toys</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Baby Care</h6>

                    <li><a href="/">Diapers</a></li>
                    <li><a href="/">Wipes</a></li>
                    <li><a href="/">Baby Grooming</a></li>
                    <li><a href="/">Baby Food</a></li>
                    <li><a href="/">Baby Gear</a></li>
                    <li><a href="/">Baby Beeding</a></li>
                    <li><a href="/">Baby Oral Care</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Featured Brands</h6>

                    <li><a href="/">Miss & Chief</a></li>
                    <li><a href="/">Barbie</a></li>
                    <li><a href="/">Disney</a></li>
                    <li><a href="/">US Polo</a></li>
                    <li><a href="/">Crocs</a></li>
                    <li><a href="/">Lego</a></li>
                    <li><a href="/">Puma</a></li>
                </div>
             </div>
           </div>

           <div className='one-opt'>
             <span className='six'>Home & Furniture <KeyboardArrowDownIcon fontSize="small" className="down-iscon"/></span>
             <div className='down-side'>
                <div className='down-hill'>
                    <h6 className='sire'>Kitchen Storage</h6>

                    <li><a href="/">Water Bottles</a></li>
                    <li><a href="/">Lunch Boxes</a></li>
                    <li><a href="/">Flasks</a></li>
                    <li><a href="/">Casseroles</a></li>
                    <li><a href="/">Kitchen Containers</a></li>
                    <li><a href="/">Cleaning Supplies</a></li>
                    <li><a href="/">Dinner Set</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Living Room Furniture</h6>

                    <li><a href="/">Sofa</a></li>
                    <li><a href="/">Sofa Beds</a></li>
                    <li><a href="/">TV Units</a></li>
                    <li><a href="/">Dining Tables</a></li>
                    <li><a href="/">Chairs</a></li>
                    <li><a href="/">Coffee Tables</a></li>
                    <li><a href="/">Shoe Racks</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Furnishing</h6>

                    <li><a href="/">Beedsheets</a></li>
                    <li><a href="/">Curtains</a></li>
                    <li><a href="/">Cushion & Pillows</a></li>
                    <li><a href="/">Blankets</a></li>
                    <li><a href="/">Bath & Towel</a></li>
                    <li><a href="/">Kitch & Table Linen</a></li>
                    <li><a href="/">Floor Coverings</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Home Decor</h6>

                    <li><a href="/">Paintings</a></li>
                    <li><a href="/">Clocks</a></li>
                    <li><a href="/">Wall Shelves</a></li>
                    <li><a href="/">Sticker</a></li>
                    <li><a href="/">Showpieces</a></li>
                    <li><a href="/">Flugrines</a></li>
                    <li><a href="/">Wall Lamp</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Gardening Store</h6>

                    <li><a href="/">Stainless Steel Store</a></li>
                    <li><a href="/">Milton</a></li>
                    <li><a href="/">Bombay Dyeing</a></li>
                    <li><a href="/">Home Town</a></li>
                    <li><a href="/">Ajanta</a></li>
                    <li><a href="/">Prestige</a></li>
                    <li><a href="/">Spaces By Welspun</a></li>
                </div>
             </div>
           </div>

           <div className='one-opt'>
             <span className='seven'>Sports,Books&More<KeyboardArrowDownIcon fontSize="small" className="down-iscon"/></span>
             <div className='down-side'>
                <div className='down-hill'>
                    <h6 className='sire'>Sports</h6>

                    <li><a href="/">Cricket</a></li>
                    <li><a href="/">Basketball</a></li>
                    <li><a href="/">Football</a></li>
                    <li><a href="/">Skating</a></li>
                    <li><a href="/">Swimming</a></li>
                    <li><a href="/">Hiking</a></li>
                    <li><a href="/">Camping</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Food Essentials</h6>

                    <li><a href="/">Nuts & Dry Fruits</a></li>
                    <li><a href="/">Chocolates</a></li>
                    <li><a href="/">Snack Corner</a></li>
                    <li><a href="/">Gifting Combos</a></li>
                    <li><a href="/">Sweets Store</a></li>
                    <li><a href="/">Breakfast items</a></li>
                    <li><a href="/">Tea Coffee</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Books</h6>

                    <li><a href="/">Entrance Exams</a></li>
                    <li><a href="/">Academics</a></li>
                    <li><a href="/">Non Fiction</a></li>
                    <li><a href="/">Young Readers</a></li>
                    <li><a href="/">Self-Help</a></li>
                    <li><a href="/">E-Learning</a></li>
                    <li><a href="/">Preorder</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Auto Accessories</h6>

                    <li><a href="/">Car Audio/Video</a></li>
                    <li><a href="/">Car Mobile Accessories</a></li>
                    <li><a href="/">Car & Bike Car</a></li>
                    <li><a href="/">Helmets</a></li>
                    <li><a href="/">Riding Gears</a></li>
                    <li><a href="/">Vechile Lubricants</a></li>
                    <li><a href="/">Safety Products</a></li>
                </div>

                <div className='down-hill'>
                    <h6 className='sire'>Gaming</h6>

                    <li><a href="/">Gaming Consoles</a></li>
                    <li><a href="/">Gaming Accessories</a></li>
                    <li><a href="/">PS4 Games</a></li>
                    <li><a href="/">Smart Glasses (VR)</a></li>
                    <li><a href="/">Grocery (Only in Select Cities)</a></li>
                    
                </div>
             </div>
           </div>

           <div className='one-opt'>
             <span className='eight'>Flights</span>
           </div>

           <div className='one-opt'>
             <span className='nine'>Offer Zone</span>
           </div>

         </div>


       </div>
    )
}

export default MenFootwear
